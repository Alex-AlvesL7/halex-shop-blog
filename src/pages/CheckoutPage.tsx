      ...
      <div className="max-w-7xl mx-auto px-6 py-8 md:py-10">
        <div className={`mb-8 rounded-[28px] px-5 py-4 md:px-6 md:py-5 flex items-center justify-between gap-4 flex-wrap ${heroCardClasses}`}>
          <div className="flex items-center gap-4">
            <button onClick={onBack} className={`inline-flex items-center gap-2 px-4 py-2 rounded-2xl transition-colors ${isDark ? 'bg-white/5 hover:bg-white/10 text-white' : 'border border-gray-200 hover:border-brand-orange hover:text-brand-orange'}`}><ArrowLeft size={18} /> Voltar ao carrinho</button>
            <button onClick={onBack} className={`text-2xl font-display font-black tracking-tighter transition-colors ${isDark ? 'text-white hover:text-brand-orange' : 'text-brand-black hover:text-brand-orange'}`}>HALEX<span className="text-brand-orange">SHOP</span></button>
          </div>
          {themeToggle}
        </div>
        ...
        <div className="mb-8 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <div className={`flex items-center gap-2 text-xs font-bold uppercase tracking-[0.3em] mb-4 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}><span>Contato</span><ChevronRight size={14} /><span>Entrega</span><ChevronRight size={14} /><span>Pagamento</span></div>
          ...
          <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-8 items-start">
            <div className="space-y-6">
              ...
              <div className={`rounded-[32px] p-8 ${sectionCardClasses}`}>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <label className="block"><span className={labelClasses}>CEP</span><input value={formData.cep} onChange={(e) => onFieldChange('cep', formatCep(e.target.value))} onBlur={handleCepBlur} className={inputClasses} placeholder="00000-000" /><span className={`text-xs mt-2 block ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{cepLoading ? 'Buscando endereço pelo CEP...' : 'Somente endereços do Brasil'}</span></label>
                  <label className="block"><span className={labelClasses}>País</span><input value="Brasil" disabled className={`w-full px-4 py-3 border-2 rounded-2xl ${isDark ? 'border-white/5 bg-[#171717] text-gray-400' : 'border-gray-100 bg-gray-50 text-gray-500'}`} /></label>
                  ...
                  <label className="block md:max-w-[180px]"><span className={labelClasses}>UF</span><select value={formData.state} onChange={(e) => onFieldChange('state', e.target.value)} className={`w-full px-4 py-3 border-2 rounded-2xl focus:border-brand-orange focus:outline-none ${isDark ? 'border-white/10 bg-[#171717] text-white' : 'border-gray-200 bg-white'}`}><option value="">Selecione</option>{brazilStates.map((state) => <option key={state} value={state}>{state}</option>)}</select></label>
                </div>
              </div>
            </div>
            ...
            <div className="lg:sticky lg:top-24 space-y-6">
              <div className={`rounded-[32px] p-8 shadow-2xl ${isDark ? 'bg-white text-brand-black' : 'bg-brand-black text-white'}`}>
                <div className="flex items-center gap-3 mb-6"><CreditCard size={22} className="text-brand-orange" /><div><h2 className="text-2xl font-black">Resumo do pedido</h2><p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Confira seus dados antes de seguir para o pagamento.</p></div></div>
                <div className={`space-y-4 pb-6 mb-6 ${isDark ? 'border-b border-black/10' : 'border-b border-white/10'}`}>{cart.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 text-sm"><div><p className="font-bold">{item.quantity}x {item.name}</p><p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>R$ {item.price.toFixed(2)} cada</p></div><span className="font-black">R$ {(item.price * item.quantity).toFixed(2)}</span></div>)}</div>
                <div className={`space-y-3 text-sm pb-6 mb-6 ${isDark ? 'border-b border-black/10' : 'border-b border-white/10'}`}><div className="flex items-center justify-between"><span className={isDark ? 'text-gray-500' : 'text-gray-400'}>Subtotal</span><span>R$ {productsTotal.toFixed(2)}</span></div><div className="flex items-center justify-between"><div><p className={isDark ? 'text-gray-500' : 'text-gray-400'}>Frete</p>{selectedFrete && <p className={`text-xs ${isDark ? 'text-gray-600' : 'text-gray-500'}`}>{selectedFrete.carrier?.name || selectedFrete.company?.name} {selectedFrete.name}</p>}</div><span>R$ {freteValue.toFixed(2)}</span></div></div>
                ...
                <div className={`mt-4 text-xs leading-relaxed ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Seus dados de contato e entrega serão vinculados ao pedido e poderão ser consultados depois no painel administrativo.</div>
              </div>

              {selectedFrete && <div className={`rounded-[28px] p-6 ${sectionCardClasses}`}><p className={`text-xs font-bold uppercase tracking-widest mb-2 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Frete selecionado</p><p className={`font-black text-lg ${titleClasses}`}>{selectedFrete.carrier?.name || selectedFrete.company?.name} {selectedFrete.name}</p><p className="text-brand-orange font-black text-2xl mt-2">R$ {freteValue.toFixed(2)}</p><p className={`text-sm mt-1 ${mutedTextClasses}`}>Entrega estimada em {selectedFrete.delivery_time} dia(s).</p></div>}
            </div>
          </div>
        )}
      </div>
